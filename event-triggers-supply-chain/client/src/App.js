...
class App extends Component {
  ...
  listenToPaymentEvent = () => {
    ...
    this.itemManager.events.SupplyChainStep().on("data", async function(event) {
      console.log(event);
      const itemObj = self.itemManager.methods.items(event.returnValues._itemIndex).call();
  ...
  render() {
    ...
    return (
      <div className="App">
        <h1>Event Trigger / Suppy Chain Example</h1>
        <h2>Items</h2>
        <h2>Add Items</h2>
        <label>Cist in Wei: <input type="text" name="cost" value={this.state.cost} onChange={this.handleInputChange}/></label>
        <label>Item Identifier: <input type="text" name="itemName" value={this.state.itemName} onChange={this.handleInputChange}/></label>
